...
import com.example.mytokdismovieapp.ui.detail.DetailMoviePopularActivity
import com.example.mytokdismovieapp.utils.DateHelper
import kotlinx.android.synthetic.main.movie_item.view.*

class MoviePopularAdapter(private var resultsItemMovie: MutableList<ResultsItemMovie>, private val resultItemGenre: MutableList<ResultsItemGenre>, private val listener: (ResultsItemMovie) -> Unit) : PagedListAdapter<ResultsItemMovie, MoviePopularAdapter.MovieViewHolder>(DIFF_CALLBACK) {
    ...
    inner class MovieViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun bind(resultMovie: ResultsItemMovie, listener: (ResultsItemMovie) -> Unit) {
            ...
            itemView.setOnClickListener {
                val intent = Intent(itemView.context, DetailMoviePopularActivity::class.java)
                intent.putExtra(DetailMoviePopularActivity.EXTRA_MOVIE, resultMovie)
                intent.putExtra(DetailMoviePopularActivity.EXTRA_GENRE, resultMovie.genreIds?.let { getGenres(it) })
